...
// UnexpectedTokenError represents an error that occurs when the parser expects the next
// token to be something, but it isn't.
...
// InvalidTokenError occurs when there is a token where it shouldn't be. It is like
// UnexpectedTokenError, but it doesn't need an Expected field.
...
// IntegerParseError represents an error that occurs when trying to parse an string
...
// NoPrefixParseFnError represents an error that occurs when the parser encounters a
// token it doesn't have a prefix parse function for.
